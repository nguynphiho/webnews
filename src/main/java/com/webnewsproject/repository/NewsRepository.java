...
public interface NewsRepository extends PagingAndSortingRepository<News, Integer> {
    ...
    @Query(nativeQuery = true,value = "select top 3 * from News n where n.category_id = ?1 order by n.date_upload desc")
    List<News> findTopByCategoryId(int categoryId);

    @Query(nativeQuery = true,value = "select top 3 * from News n where n.status_id = ?1 order by n.date_upload desc")
